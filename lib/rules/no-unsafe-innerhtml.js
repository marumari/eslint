...
module.exports = function (context) {
  ...
  var allowedExpression = function allowedExpression(expression) {
   ...
   */
    if (expression.type === 'Literal') { return true; } // we just assign a literal (e.g. a string, a number, a bool)

    else if (expression.type === 'TemplateLiteral') {

      // check for ${..} expressions
      if (expression.expressions.length === 0) { return true; }
      else { return false; } // else: contains expressions, but no tagged function? not cool.
    }

    else if (expression.type === 'TaggedTemplateExpression') {
      // context.getSource(expression.tag) is the function name
      if (VALID_ESCAPERS.indexOf(context.getSource(expression.tag)) !== -1) { return true; }
      else { return false; }
    }

    else { return false; } // everything that doesn't match is unsafe
  };
